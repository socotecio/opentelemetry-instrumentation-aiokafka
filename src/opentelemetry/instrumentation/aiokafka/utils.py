...
class KafkaPropertiesExtractor:
    ...
    def extract_bootstrap_servers(instance):
        # INFO - AM - Producer have a client key and consumer a _client key. Fuck inconsistency.
        aio_kafka_client = getattr(instance, "client", None)
        if aio_kafka_client is None:
            aio_kafka_client = getattr(instance, "_client", None)

        return aio_kafka_client._bootstrap_servers
    ...
    def extract_send_headers(args, kwargs):
        ...
        return KafkaPropertiesExtractor._extract_argument("headers", 5, None, args, kwargs)
    ...
    def extract_send_partition(instance, args, kwargs):
        ...
        try:
            ...
            key_bytes = instance._serialize(instance._key_serializer, topic, key)
            value_bytes = instance._serialize(instance._value_serializer, topic, value)
            ...
            instance._wait_on_metadata(topic, instance._request_timeout_ms / 1000.0)
...
def _wrap_send(tracer: Tracer, produce_hook: ProduceHookT) -> Callable:
    def _traced_send(func, instance, args, kwargs):
        ...
        with tracer.start_as_current_span(span_name, kind=trace.SpanKind.PRODUCER) as span:
            ...
            try:
                # INFO - AM - 21/02/2023 - produce_hook is None by default. Nothing to look at here
                if callable(produce_hook):
                    produce_hook(span, args, kwargs)
            except Exception as hook_exception:  # pylint: disable=W0703
                _LOG.exception(hook_exception)
        # INFO - AM - 21/02/2023 - removing headers from args as we pass it as kwargs
        if len(args) > 5:
            args = args[:-1]
